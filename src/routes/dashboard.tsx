  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700">
                ...
                return (
                  <tr key={tx.id} className={`hover:bg-slate-700/50 ${isVoided ? 'opacity-50' : ''}`}>
                    ...
                    <td className="py-3 text-right">
                      <span className="px-2 py-1 bg-slate-700 rounded text-xs text-slate-300">{tx.payment_method}</span>
                    </td>
                    <td className="py-3 text-right">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${isVoided ? 'bg-red-900/30 text-red-400' : 'bg-emerald-900/30 text-emerald-400'}`}>
                        {tx.status}
                      </span>
                    </td>
                    {canVoidSales && (
                      <td className="py-3 text-right">
                        {tx.status === 'completed' && (
                          <button
                            onClick={() => { setVoidTransaction(tx); setShowVoidModal(true); }}
              ...
              {recentTransactions.length === 0 && (
                <tr>
                  <td colSpan={canVoidSales ? 7 : 6} className="py-8 text-center text-slate-500 text-sm">No transactions yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <VoidTransactionModal
        ...
        onClose={() => { setShowVoidModal(false); setVoidTransaction(null); }}
        onVoidComplete={() => { setShowVoidModal(false); setVoidTransaction(null); loadData(); }}
      />
    </div>
  );
}
