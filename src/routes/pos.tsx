...
export function PosPage() {
  ...
  async function handleCheckout() {
    ...
    try {
      await createTransaction(
        { customer_id: customerId, cashier_id: user?.id ?? null, total_amount: cartTotal, amount_paid: paymentMethod === 'cash' ? paid : cartTotal, change_due: paymentMethod === 'cash' ? changeDue : 0, payment_method: paymentMethod, sale_type: saleType, status: 'completed' },
        cart.map((item) => ({ product_id: item.product.id, product_name: item.product.name, quantity: item.quantity, unit_price: item.unitPrice, subtotal: item.unitPrice * item.quantity }))
      );
      setSuccess(`Sale completed: KES ${cartTotal.toLocaleString()}`);
      setCart([]); setAmountPaid(''); setCustomerId(null); setPaymentMethod('cash'); setSaleType('standard');
      await loadData();
  ...
  return (
    <div className="space-y-4">
      ...
      {success && <div className="rounded-lg bg-emerald-900/30 border border-emerald-800 p-4 text-emerald-300 flex items-center gap-2"><CheckCircle size={18} /> {success}</div>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search products by name or SKU..." className="w-full rounded-lg bg-slate-800 border border-slate-700 text-white pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500" />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {filteredProducts.map((product) => (
              <button key={product.id} onClick={() => addToCart(product)} disabled={product.stock_quantity <= 0} className="text-left rounded-xl bg-slate-800 border border-slate-700 p-4 hover:border-emerald-600 transition disabled:opacity-40 disabled:cursor-not-allowed">
        ...
        <div className="bg-slate-800 rounded-xl border border-slate-700 p-5 space-y-4">
          <div className="flex items-center gap-2"><ShoppingCart className="text-emerald-400" size={20} /><h2 className="text-lg font-semibold text-white">Cart</h2>{cart.length > 0 && <span className="ml-auto text-xs bg-emerald-600 text-white px-2 py-0.5 rounded-full">{cart.length}</span>}</div>
          <div>
            ...
            <select value={saleType} onChange={(e) => handleSaleTypeChange(e.target.value as SaleType)} className="w-full rounded-lg bg-slate-900 border border-slate-700 text-white px-3 py-2 text-sm focus:outline-none focus:border-emerald-500">
              <option value="standard">Standard</option><option value="wholesale">Wholesale</option>
            </select>
          </div>
          <div>
            ...
            <select value={customerId ?? ''} onChange={(e) => setCustomerId(e.target.value || null)} className="w-full rounded-lg bg-slate-900 border border-slate-700 text-white px-3 py-2 text-sm focus:outline-none focus:border-emerald-500">
              ...
              {customers.filter((c) => c.name !== 'Walk-in Customer').map((c) => (<option key={c.id} value={c.id}>{c.name}</option>))}
            </select>
          </div>
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {cart.map((item) => (
              <div key={item.product.id} className="flex items-center gap-2 bg-slate-900 rounded-lg p-2">
                <div className="flex-1 min-w-0"><p className="text-white text-xs font-medium truncate">{item.product.name}</p><p className="text-slate-500 text-xs">KES {item.unitPrice.toLocaleString()} × {item.quantity}</p></div>
          ...
          <div className="border-t border-slate-700 pt-3 space-y-3">
            <div className="flex justify-between"><span className="text-slate-400 text-sm">Total</span><span className="text-white font-bold text-lg">KES {cartTotal.toLocaleString()}</span></div>
            <div>
