...
import { Save, Loader as Loader2, CircleCheck as CheckCircle, CircleAlert as AlertCircle, Smartphone, Eye, EyeOff, Power } from 'lucide-react';
...
export function SettingsPage() {
  ...
  // THE FIX: On mount, load settings from Supabase (not localStorage).
  // This ensures previously saved settings are always populated,
  // even after the app stops and restarts.
  useEffect(() => {
    loadSettings();
  }, []);
  ...
  async function handleSave() {
    ...
    setSuccess(false);
    try {
      // Save to Supabase — settings persist across app restarts
      const updated = await saveMpesaSettings({
        ...
        security_credential: form.security_credential,
      });
      setSettings(updated);
      setSuccess(true);
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">KCB BUNI M-Pesa Express API — STK Push configuration</p>
      </div>

      {success && (
        <div className="rounded-lg bg-emerald-900/30 border border-emerald-800 p-4 text-emerald-300 flex items-center gap-2">
          <CheckCircle size={18} />
          Settings saved successfully. They will persist across app restarts.
        </div>
      )}
      {error && (
        <div className="rounded-lg bg-red-900/30 border border-red-800 p-4 text-red-300 flex items-center gap-2">
          <AlertCircle size={18} />
          {error}
        </div>
      )}
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-6 space-y-5">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-700">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${form.enabled ? 'bg-emerald-600' : 'bg-slate-700'}`}>
              <Smartphone className="text-white" size={20} />
            </div>
            <div>
              <p className="text-white font-medium text-sm">M-Pesa STK Push</p>
              <p className="text-xs text-slate-500">Enable to accept M-Pesa payments at checkout</p>
            </div>
          </div>
          <button
            onClick={handleToggleEnabled}
            className={`relative w-12 h-6 rounded-full transition ${form.enabled ? 'bg-emerald-600' : 'bg-slate-600'}`}
          >
            <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white transition-transform ${form.enabled ? 'translate-x-6' : ''}`} />
          </button>
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-700">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex items-center gap-2 rounded-lg bg-emerald-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-emerald-700 transition disabled:opacity-50"
          >
