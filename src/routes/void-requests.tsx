...
export function VoidRequestsPage() {
  ...
  async function loadRequests() {
    try {
      const data = await getAllVoidRequests();
      setRequests(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  async function handleApprove(vr: VoidRequest) {
    ...
    setError(null);
    try {
      await approveRequest(vr.approval_request_id, user);
      await loadRequests();
    } catch (err) {
  ...
  async function handleReject() {
    if (!user || !rejecting || !rejecting.approval_request_id) return;
    if (!rejectReason.trim()) return;
    setActionLoading(rejecting.id);
    setError(null);
    try {
      await rejectRequest(rejecting.approval_request_id, user, rejectReason.trim());
      setRejecting(null);
      setRejectReason('');
      await loadRequests();
    } catch (err) {
      ...
      <div className="space-y-3">
        ...
        {pending.length === 0 ? (
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-12 text-center">
            <FileX className="mx-auto mb-3 text-slate-600" size={32} />
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-2 flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-amber-400 text-xs font-medium uppercase">Pending</span>
                    <span className="text-slate-500 text-xs">
                      {new Date(vr.created_at).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex items-baseline gap-3">
                    <p className="text-white font-bold text-lg">KES {Number(vr.transaction_total).toLocaleString()}</p>
                    <span className="text-xs text-slate-500">{vr.transaction_payment_method}</span>
                  </div>
                  <p className="text-slate-300 text-sm">
                    <span className="text-slate-500">Reason:</span> {vr.reason}
                  </p>
                  <p className="text-xs text-slate-500">
                    Requested by {vr.requester_name} ({vr.requester_role})
                  </p>
                </div>
                ...
                {canApprove && (
                  <div className="flex flex-col gap-2 shrink-0">
                    ...
                    >
                      {actionLoading === vr.id ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />}
                      Approve
                    </button>
                    <button
                      onClick={() => { setRejecting(vr); setRejectReason(''); }}
                      disabled={actionLoading === vr.id}
                      className="flex items-center gap-2 rounded-lg border border-red-700 px-3 py-2 text-xs font-medium text-red-400 hover:bg-red-900/20 transition disabled:opacity-50"
                    >
                      <X size={14} /> Reject
                    </button>
                  </div>
                ...
                <div className="space-y-1 flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-medium uppercase ${
                      vr.status === 'completed' ? 'text-emerald-400' :
                      vr.status === 'rejected' ? 'text-red-400' : 'text-slate-400'
                    }`}>
                      {vr.status}
                    </span>
                    <span className="text-slate-500 text-xs">
                  ...
                  <p className="text-xs text-slate-500">
                    Requested by {vr.requester_name} · {vr.approver_name ? `Approved/rejected by ${vr.approver_name}` : ''}
                  </p>
                  {vr.rejection_reason && (
                    <p className="text-xs text-red-400 mt-1">Rejection reason: {vr.rejection_reason}</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setRejecting(null)}>
          <div className="w-full max-w-md rounded-2xl bg-slate-800 border border-slate-700 p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-400 mb-3">
              Provide a reason for rejecting this void request for KES {Number(rejecting.transaction_total).toLocaleString()}.
            </p>
            <textarea
              value={rejectReason}
              ...
              className="w-full rounded-lg bg-slate-900 border border-slate-700 text-white px-3 py-2 text-sm focus:outline-none focus:border-emerald-500 mb-4"
            />
            <div className="flex gap-3">
              ...
              >
                {actionLoading === rejecting.id ? 'Rejecting...' : 'Reject Request'}
              </button>
            </div>
          </div>
        </div>
