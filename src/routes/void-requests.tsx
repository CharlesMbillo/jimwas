  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">{pending.length} pending · {resolved.length} resolved</p>
      </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-2 flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-500 text-xs">{new Date(vr.created_at).toLocaleString()}</span>
                  </div>
                ...
                {canApprove && (
                  <div className="flex flex-col gap-2 shrink-0">
                    <button onClick={() => handleApprove(vr)} disabled={actionLoading === vr.id} className="flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-medium text-white hover:bg-emerald-700 transition disabled:opacity-50">
                      {actionLoading === vr.id ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} Approve
                    </button>
                    <button onClick={() => { setRejecting(vr); setRejectReason(''); }} disabled={actionLoading === vr.id} className="flex items-center gap-2 rounded-lg border border-red-700 px-3 py-2 text-xs font-medium text-red-400 hover:bg-red-900/20 transition disabled:opacity-50">
                      <X size={14} /> Reject
                    </button>
                  </div>
                )}
            ...
            <div key={vr.id} className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1 flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-medium uppercase ${vr.status === 'completed' ? 'text-emerald-400' : vr.status === 'rejected' ? 'text-red-400' : 'text-slate-400'}`}>{vr.status}</span>
                    <span className="text-slate-500 text-xs">{new Date(vr.created_at).toLocaleDateString()}</span>
                  </div>
